...
flowchart TD
    ...
    B -- No --> D{Complex question?<br/>COMPLEX_TRIGGERS}
    D -- Yes --> E[Opus + ccc search]
    D -- No --> F{Question about the assistant?<br/>ASSISTANT_TRIGGERS}
    F -- Yes --> G[Sonnet answers directly]
    F -- No --> H[ccc search]
    H --> I{Sources above<br/>threshold?}
    I -- No --> J[Sonnet<br/>without Haiku]
    I -- Yes --> K[Haiku classifies]
    K --> L{Classification}
    L -- complex --> M[Opus]
    L -- simple --> N[Sonnet]
    L -- irrelevant --> O[Default response<br/>without Claude]
```
